...
namespace Razor.XML
{
    ...
    public static class FileSerializer
    {
        ...
        public static void SerializeToXmlFile<T>(T target, string file)
        {
            ...
            {
                throw new Exception(string.Format("将对象序列化为指定XML文件异常,{0}", ex.Message + Environment.NewLine + ex.StackTrace));
            }
        }
        ...
        public static T DeserializeFromXmlFile<T>(string file)
        {
            ...
            {
                throw new Exception(string.Format("方法调用异常,{0}", ex.Message + Environment.NewLine + ex.StackTrace));
            }
            catch (FileLoadException ex)
            {
                throw new Exception(string.Format("文件加载异常,{0}", ex.Message));
            }
            catch (FileNotFoundException ex)
            {
                throw new Exception(string.Format("未找到文件,{0}", ex.Message));
            }
