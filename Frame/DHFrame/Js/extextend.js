...
/*选择窗口，返回选中的record
*   以下参数参数必填： 
*   @hideBbar: false,        //是否隐藏bbar
*   @iframeScroll: false,    //指定iframe是否有滚动条
*   @srcF: "",               //扩展属性iframe 地址
*   @gridOrTreeId: "",       //扩展属性gridortree控件ID
*   事件：
*   @winclosed               //关闭窗口时返回的事件，arg1:recoreds.arg2:scrope
*/
Ext.ns("Ext.ux");
Ext.define('Ext.ux.SelectWin', {
    ...
    constructor: function (cfg) {
        cfg = cfg || {};
        this.srcF = cfg.srcF;
        this.iframeScroll = cfg.iframeScroll || false;
        this.hideBbar = cfg.hideBbar || false;
    ...
    listeners: {
        'afterRender': {
            fn: function () {
                //var myMask = new Ext.LoadMask(this.getComponent("tempPanel"), { useMsg: false });
                //myMask.show();
            }, scope: this
        }
    },
    bbar: {
        style: {
            width: '95%',
            borderTop: "solid 1px #ccc !important",
            backgroundColor: '#fff'
        },
        height: 40,
        items: []
    ...
    initComponent: function () {
        //添加自定义事件
        this.addEvents('winclosed');
        var me = this;

        //-----iframe初始化--------------------
        var src = me.srcF;  //设置URL
        var tWidth = parseInt(me.width) - 10; //设置frame内容宽度
        var tHeight = parseInt(me.height) - 85;//这里高度根据需要调整
        var isScroll = me.iframeScroll ? "1" : "no";  //设置iframe是否有滚动条
        me.items = [];
        me.items.push({
            region: "center",
            border: false,
            ...
            frame: false,
            autoScroll: true,
            html: '<iframe width="' + tWidth + '" height="' + tHeight + '" src="' + src + '" scrolling="' + isScroll + '" frameborder="0" frameborder="0" id="contentFrame" name="contentFrame" ></iframe>'
        })

        if (!me.hideBbar) {
            var okBtn = Ext.create('Ext.Button', {
                width: 60,
                icon: "/icons/disk-png/ext.axd",
                height: 27,
                text: '确定',
                handler: function () {
                    var frame = window.contentFrame; //当前窗口iFrame Name
                    var recds = frame.Ext.getCmp(me.gridOrTreeId).getSelectionModel().getSelection();
                    if (Ext.isArray(recds) && recds.length > 0) {
                        me.fireEvent("winclosed", recds, me);
                    } else {
                        Ext.Msg.alert("提示", "请选择数据!");
                    }
                }
            });
            var okCancel = Ext.create('Ext.Button', {
                icon: "/icons/cancel-png/ext.axd",
                width: 60,
                height: 27,
                text: '取消',
                handler: function () {
                    me.fireEvent("winclosed", null, me);
                    me.close()
                }
            });
            this.bbar.items = [];
            this.bbar.items.push("->", okBtn, okCancel);
        } else {
            //隐藏底部工具栏
            me.bbar = null;
        }
    ...
    //设置选中的值
    setSelectedVal: function (recrds) {
        this.fireEvent("winclosed", recrds, this);
        // this.close()
    },
    //关闭窗口
    closeWin: function () {
...
//*************************RazorStore*****************
/*
*   封装后的RazorStore,首次读取数据不需要异步加载
*   @Author:    WGM
*   @Date  :    2015-5-4
*   @PageData:  本地数据
*/
Ext.define("Ext.data.RazorStore", {
    extend: 'Ext.data.Store',
    alias: 'store.razorStore',
    proxy: {
        type: 'memory',
        reader: {
            type: 'json',
    ...
    constructor: function (config) {
        ...
        this.isClent = (config["isClent"] == true);   // 默认false

        if (!this.isClent) {
            config.proxy = this.proxy;
            this.callParent([config]);
        }
        this.setProxy(new Ext.data.proxy.Ajax(
            Ext.applyIf({
                url: '?action=read',
                reader: {
                    type: 'json',
...
Ext.define('Ext.net.TagLabel', {
    ...
    valueDelimeter: "::",
    tagsDelimeter: ",",
    renderTpl: ['<ul class="x-taglabel-list">', '{[this.empty(values)]}', '{[this.tagInput()]}', '</ul>', {
        compiled: true,
        disableFormats: true,
        label: this,
        tagInput: function () {
    ...
    initComponent: function () {
        this.addEvents("beforetagadd", "tagadd", "beforetagremove", "tagremove", "click", "dblclick", "contextmenu", "beforeselect", "select");
        this.ignoreChange = 0;
    ...
    getTagObject: function (tag) {
        ...
        if ((tag.selected && (this.selectionMode == "none" || !this.selectionMode)) || (tag.selected && this.selectionMode == "single" && this.getSelected().length > 0)) {
            tag.selected = false;
        }
        var cls = ["x-taglabel-item", tag.iconCls ? "x-taglabel-item-icon" : "", tag.iconCls && tag.iconCls.indexOf('#') === 0 ? X.net.RM.getIcon(tag.iconCls.substring(1)) : (tag.iconCls || ""), tag.disabled ? "x-taglabel-item-disabled" : "", tag.cls || "", tag.selected ? "x-taglabel-item-selected" : ""],
		o;
    ...
    onclick: function (tag, e) {
        ...
        if (!tag.disabled && mode && mode != "none" && !((mode == "single" || mode == "multi") && !e.ctrlKey && tag.selected) && this.fireEvent("beforeselect", this, tag, e) !== false) {
            if (mode == "multi") {
...
Ext.define('Ext.net.TagField', {
    ...
    initTagsStore: function () {
        if (Ext.isArray(this.store)) {
            ...
            return new Ext.data.Store({
                data: this.store,
                fields: ["text", "value", "selected", "iconCls", "disabled", "cls", "qTitle", "qTip", "width", "style", "closable", "overCls"],
                autoDestroy: true
            });
        }
        return this.store;
    ...
    initEvents: function () {
        ...
        me.mon(me.inputEl, 'paste', me.onInputElPaste, me, {
            buffer: 50
        });
